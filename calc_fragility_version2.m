function [fragility,wiring_cost_mean,wiring_cost_std,P_mean,S_mean,S_std,nodes_strength,W_fracs,hubs] = calc_fragility_version2(W,fiberdist,thrs,nfracs,nhubs,nruns,thrs_hub)
% calc_fragility.m
%
...
%          nodes_strength : strength of all nodes [N x nfracs x nruns]
...
wiring_cost = zeros(N,N,nfracs,nruns);
...
for frac_ind=1:nfracs
    for run_ind=1:nruns
        ...
        % wiring cost
        wiring_cost(:,:,frac_ind,run_ind) = Gs.*fiberdist;
...
wiring_cost_mean = mean(wiring_cost,4);
wiring_cost_std = std(wiring_cost,0,4);
