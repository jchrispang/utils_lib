function ami = calc_ami(parc_1, parc_2)
% calc_ami.m
%
...
%
% Output: AMI : adjusted mutual information (float)
%
% Original: James Pang, Monash University, 2023

%%

parc_1 = relabel_parcellation(parc_1);
parc_2 = relabel_parcellation(parc_2);
num_parcels_1 = length(unique(parc_1(parc_1>0)));
num_parcels_2 = length(unique(parc_2(parc_2>0)));

nonzero_parc_1 = find(parc_1>0);
nonzero_parc_2 = find(parc_2>0);
nonzero = intersect(nonzero_parc_1, nonzero_parc_2);

ami = AMI(parc_1(nonzero), parc_2(nonzero));

if num_parcels_1 == 1 || num_parcels_2 == 1
    ami = 1;
end
if isnan(ami)
    ami = 0;
...
%%% THIS PORTION WAS ADOPTED FROM VINH ET AL. (2009)
% Xuan Vinh Nguyen (2023). The Adjusted Mutual Information (https://www.mathworks.com/matlabcentral/fileexchange/33144-the-adjusted-mutual-information), 
% MATLAB Central File Exchange. Retrieved February 20, 2023.
