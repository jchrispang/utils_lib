...
function [h, u] = raincloud_plot_James(X, varargin)
...
% quartiles   = quantile(X, [0 1 0.5]);     % JAMES EDIT
iqr         = quartiles(2) - quartiles(1);
Xs          = sort(X);
% whiskers(1) = min(Xs(Xs > (quartiles(1) - (1.5 * iqr))));
% whiskers(2) = max(Xs(Xs < (quartiles(2) + (1.5 * iqr))));
whiskers(1) = min(Xs(Xs > (quartiles(1) - (3 * iqr))));
whiskers(2) = max(Xs(Xs < (quartiles(2) + (3 * iqr))));
